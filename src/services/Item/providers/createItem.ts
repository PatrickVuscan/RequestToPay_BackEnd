...
const generateSetString: (item: Item) => string = (item: Item) => {
    return `INSERT INTO "RequestToPay"."Item"
        ("IID", "Name", "SID", "Price")
        VALUES (default, '${item.Name}', ${item.SID}, ${item.Price})
        RETURNING "IID"`;
};
...
export const createItem: (item: Item) => Promise<number> = async (item: Item) => {
    let res = null;
    try {
        res = await q(generateSetString(item));
    } catch (e) {
